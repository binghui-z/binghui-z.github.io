...
\begin{itemize}
  \item 针对双手交互造成的严重遮挡问题，提出了一种新的关节热图表达方式。比起传统的关节热图，我们的方法能够将交互区域的关节自适应地映射到热图上；
  \item 现有的方法往往是在Interhand2.6M数据集上以images-paired的方式进行训练，会影响模型的泛化性。我们构建的与图像无关的交互先验，能够有效利用更多的多模态数据集，进而提升泛化性；
  \item 构建了一个面向双手交互任务的数据集Twohand-500K，该数据集交互状态更加多样，更加合理。
\end{itemize}
% \vspace{-2mm}
